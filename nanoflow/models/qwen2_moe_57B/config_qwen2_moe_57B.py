...
class Qwen2MoEConfig(PipelineConfig):
    def __init__(
        ...
        if multi_gpu_mode:
            self.pipeline_name = f"Qwen2-MoE-57B-{kv_cache_type}-{network_type}-TP{tp_size}-PP{pp_size}-DP{dp_size}-EP{ep_size}"
            self.cache_weight_name = f"Qwen2-MoE-57B-TP{tp_size}-PP{pp_size}-DP{dp_size}-EP{ep_size}"
        else:
            self.pipeline_name = f"Qwen2-MoE-57B-{kv_cache_type}"
            self.cache_weight_name = "Qwen2-MoE-57B"
