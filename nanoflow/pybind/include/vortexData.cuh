...
struct vortexUpdateData{
    ...
    // offload
    std::vector<int32_t> offloadPageIndices;
    
    // load
    std::vector<int32_t> loadPageCPUIndices;
    std::vector<int32_t> loadPageGPUIndices;
    std::vector<half*> loadPageData; // assigned by pipeline update, no need for python to update

    // load prepare
    std::vector<int32_t> loadPageCPUPrepareIndices;

    //evict offload pages
    std::vector<int32_t> evictPageIndices;
...
// output token id
    ...
    // int partial_num_2;
        // the device pointer to sampled token array, one token for each request    
    std::vector<half*> offload_kv_cache; 
        // size: num_layers, each pointer holds num_pages * page_mem_size
    int global_batch_size;
    int offload_page_num;
};
