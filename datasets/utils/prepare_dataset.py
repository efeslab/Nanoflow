...
from prepare_real_data import dataset
from prepare_synthetic_data import token_norm_dist
from prepare_splitwise_data import splitwise
from utils import get_req_time_interval
...
@click.option(
    "--request-rate",
    type=float,
    help="# of reqs/sec. 0 indicates Speed of Light/Zero-delay injection rate",
    default=0.0)
...
@click.option(
    ...
    type=int,
    default=4096,
    help="Total length of input/output combined, restriction for model context window.")
@click.pass_context
def cli(ctx, **kwargs):
