...
def dataset_dump(input_ids, output_lens, delays, metadata, output_file, mode='token'):
    if mode == 'token':
        samples = []
        ...
        index = 0
        with open(output_file, 'w') as f:
            f.write("index,input_len,output_len,time\n")
            for i in range(len(input_ids)):
                input_len=input_ids[i]
                output_len=output_lens[i]
                f.write(f"{index},{input_len},{output_len},{cur_time}\n")
                cur_time += delays[i]
                index += 1
        # workload = Workload(metadata=metadata, samples=samples)
        # with open(output_file, 'w') as f:
