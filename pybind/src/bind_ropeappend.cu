...
void splitRopeAppendWorker(torch::Tensor k_data, torch::Tensor v_data, torch::Tensor kv_indices, torch::Tensor kv_indptr, torch::Tensor kv_last_page_offset, torch::Tensor kqv_input, torch::Tensor q_global, torch::Tensor rev_input_indptr, torch::Tensor per_token_offset, int num_req, int page_size, int num_kv_heads, int num_qo_heads, int head_dim, float rope_scale, float rope_theta, float smooth_a, float smooth_b) {
    ...
    TORCH_CHECK(kv_last_page_offset.is_cuda(), "kv_last_page_offset must be a CUDA tensor");
    ...
    kv_last_page_offset = kv_last_page_offset.contiguous();
    ...
    // printf("dense_batch_size: %d\n", dense_batch_size);

    flashinfer::QKVLayout kv_layout = flashinfer::QKVLayout::kHND;

    // printf("before paged_kv\n");

    flashinfer::paged_kv_t<half, int32_t> paged_kv(
        num_kv_heads,
        page_size,
        head_dim,
        num_req, //num_req
    ...
    // printf("splitRopeAppend finished\n");
