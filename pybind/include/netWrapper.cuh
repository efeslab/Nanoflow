...
protected:
	void init(int rank, int nranks,
			  ...
			  size_t input_size, size_t output_size, size_t input_dim0, size_t input_dim1, size_t output_dim0, size_t output_dim1) {
        ...
        this->input_dim0 = input_dim0;
        this->input_dim1 = input_dim1;
        this->output_dim0 = output_dim0;
...
class NetAllGather: public NetWrapper
{
    ...
    void init(std::shared_ptr<mscclpp::Communicator> comm,
                    ...
                    size_t output_dim0,
                    size_t output_dim1) {
        NetWrapper::init(rank, nranks, input, output, input_size, output_size, input_dim0, input_dim1, output_dim0, output_dim1);
    
        std::vector<mscclpp::DeviceSyncer> syncers(nranks - 1);
        cudaMalloc(&syncersCuda, syncers.size() * sizeof(mscclpp::DeviceSyncer));
        cudaMemcpy(syncersCuda, syncers.data(), syncers.size() * sizeof(mscclpp::DeviceSyncer), cudaMemcpyHostToDevice);
    ...
    void init(std::shared_ptr<mscclpp::Communicator> comm,
                    ...
                    size_t buff_dim0,
                    size_t buff_dim1) {
            init(comm, connections, rank, nranks, buff, buff, buff_size, buff_size, buff_dim0, buff_dim1, buff_dim0, buff_dim1);
    }
    ...
    void operator()(cudaStream_t stream, int nblocks, int nthreads, bool sync) {
        ...
        if (!columnwise) {
            ...
            assert(this->input_dim0 == this->output_dim0);
            assert(this->input_dim1 == this->output_dim1 / nranks);
            const uint64_t input_ncols = this->input_dim1;
            const uint64_t output_ncols = this->output_dim1;
            const uint64_t output_row_offset = rank * input_ncols;
            const uint64_t nrows = this->input_dim0;
