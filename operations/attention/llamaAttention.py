...
class DecAttnFANoPageImpl(OperationImpl):
    ...
    def __init__(self, op_base: "DecAttn", device_id: int) -> None:
        ...
        device_id : int
            The device ID.
        """
        super().__init__(op_base, device_id)  # type: ignore
...
class PFAttnFANoPageImpl(OperationImpl):
    ...
    def __init__(self, op_base: "PFAttn", device_id: int):
...
class PFAttn(Operations):
    ...
    def update(self, qo_indicies, kv_indptr, kv_indices, kv_last_page_len, page_size,
        ...
        """
        self.qo_indicies = qo_indicies
        if self.impl.category_tag == "flash_attn_no_page":
            seq_lens = torch.diff(qo_indicies)
