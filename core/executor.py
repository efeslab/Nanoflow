...
class Executor():
    def __init__(self, operations_layers_list: list[Operation_Layer], layer_list: list[int]):
    ...
    def execute(self, output, main_stream, plan_cuda_graph=False, is_cuda_graph_enabled=False):
        if plan_cuda_graph:
            assert main_stream is not None, "stream_for_cuda_graph must be provided when use_cuda_graph is True"
            # create cuda graph
