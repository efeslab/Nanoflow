...
def process_weight_none(global_weight_map, weight_name, weight_wrapper, device_list, layer_list, weight_path, cached = False):
    return None

def process_weight_no_transpose(global_weight_map, weight_name, weight_wrapper, device_list, layer_list, weight_path, tp_size=1, tp_weights_row = False, cached = False):
    weight_wrapper.weight_map = {}
    if cached:
        for device_id in device_list:
            weight_wrapper.weight_map[device_id] = {}
            for l in layer_list:
                weight_wrapper.weight_map[device_id][l] = torch.load(f"{weight_path}/device_{device_id}/{weight_wrapper.owner.name}_{l}.pt").to(f'cuda:{device_id}')
    ...
    if not cached:
        for device_id in device_list:
            ...
            for l in layer_list:
                ...
                torch.save(weight_wrapper.weight_map[device_id][l].to("cpu"), f"{weight_path}/device_{device_id}/{weight_wrapper.owner.name}_{l}.pt")
...
def process_weight_layer(global_weight_map, weight_name, weight_wrapper, device_list, layer_list, weight_path, cached = False):
    weight_wrapper.weight_map = {}
    if cached:
        for device_id in device_list:
            weight_wrapper.weight_map[device_id] = {}
