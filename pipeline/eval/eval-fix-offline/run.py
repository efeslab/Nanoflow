...
def main():
    ...
    for trace in os.listdir(fix_trace):
        ...
        log_file = os.path.join(result_base, f"{input_len}-{output_len}-{rate}.log")
        result_file = os.path.join(result_base, f"{input_len}-{output_len}-{rate}.stat.csv")
        ...
        command = [
            ...
            f"--output_prefix={os.path.join(result_base, f'{input_len}-{output_len}-{rate}')}",
            f"--skip_cycles={2000 if int(output_len) < 10 else 10000}",
            f"--empty_weight=True",
            f"--run_cycles=1500"
        ]
