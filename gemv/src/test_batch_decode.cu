...
void _TestBatchDecodingKernelCorrectness(size_t page_size,
	...
	for(size_t i = 0; i < batch_size; ++i) {
		...
		std::vector<T> o_ref_i = cpu_reference::single_mha<T, T>(qi,
																 ki,
																 vi,
																 1,
																 seq_len,
	...
	handler.BeginForward<PageStorage::kIndices, kv_layout, T, T, int32_t>(
		(void*)thrust::raw_pointer_cast(buffer.data()),
	...
	std::cout << "page_size=" << page_size << ", num_qo_heads=" << num_qo_heads
			  ...
			  << ", launch_type=" << flashinfer::LaunchTypeToString(lType) << ", sm_blk=" << sm_blk
				...
				for(size_t num_kv_heads : {32, 8, 4}) {
					for(size_t head_dim : {64, 128, 256}) {
						for(size_t pos_encoding_mode : {0U, 1U}) {
							for(size_t launch_type : {0U, 1U}) {
								for(size_t sm_blk : {7, 11, 30, 60, 90}) {
									_TestBatchDecodingKernelCorrectness<T>(
										...
										head_dim,
										flashinfer::PosEncodingMode(pos_encoding_mode),
										flashinfer::LaunchType(launch_type),
										sm_blk,
										false);
								}
							}
