...
def get_cu_file_str(
    group_size, head_dim, kv_layout, pos_encoding_mode, dtype_in, dtype_out, idtype, ltype
):
    content = """#include <attention_impl.cuh>

namespace flashinfer {{

constexpr PageStorage page_storage = PageStorage::kIndices;

template cudaError_t BatchDecodeWithPagedKVCacheDispatched<{group_size}, {head_dim}, page_storage, {kv_layout}, {pos_encoding_mode}, {dtype_in}, {dtype_out}, {idtype}, {ltype}>(
...
if __name__ == "__main__":
    pattern = (
        ...
        r"dtypein_([a-z0-9]+)_dtypeout_([a-z0-9]+)_idtype_([a-z0-9]+)_launchtype_([a-z]+)\.cu"
    )
